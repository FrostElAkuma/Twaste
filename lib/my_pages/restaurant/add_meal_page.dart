...
class _AddMealState extends State<AddMeal> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> _validation(
        AddMealController mealController, int? typeId) async {
      ...
      String newPrice = newPriceController.text.trim();

      //int typeId = Get.find<User
      ...
      if (name.isEmpty) {
        ...
        mealController
            .addMeal(name, description, price2, newPrice2, typeId!)
            .then((status) {
    ...
    return Scaffold(
      ...
      body: GetBuilder<UserController>(builder: (userController) {
        return GetBuilder<AddMealController>(builder: (addMealController) {
          bool vendor = false;
          if (userController.userModel?.vendor_id != null) {
            vendor = true;
          }
          return vendor
                          ...
                          GestureDetector(
                            onTap: () {
                              _validation(addMealController,
                                  userController.userModel!.vendor_id);
                              tapped = true;
                            },
                            child: tapped
                                ? const CustomLoader()
              ...
              : Container(
                  child: LargeText(
                      text:
                          "Oooopaa what you doing here, unotharized, I repeat you are not Otharized"),
                );
        });
      }),
    );
